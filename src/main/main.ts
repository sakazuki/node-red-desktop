...
const HELP_NODERED_DESKTOP_URL = "https://github.com/sakazuki/node-red-desktop/";
...
class BaseApplication {
  ...
  constructor(app: App) {
    ...
    ipcMain.on("help:node-red", () => {this.onHelpWeb(HELP_NODERED_URL)});
    ipcMain.on("help:node-red-desktop", () => {this.onHelpWeb(HELP_NODERED_DESKTOP_URL)});
    ipcMain.on("help:author", () => {this.onHelpWeb(HELP_AUTHOR_URL)});
  ...
  private onReady() {
    ...
    this.create();
    this.customAutoUpdater = new CustomAutoUpdater(this.getBrowserWindow(), this.status);
