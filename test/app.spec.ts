...
test.describe("Application launch", () => {
  ...
  test("has the correct title", async () => {
    ...
    // Use toHaveTitle (Playwright retry) rather than a one-shot title() call.
    // node-red appends " : <hostname>" via red.js, so the final title is e.g.
    // "Node-RED-Desktop : 127.0.0.1" — match with a regex.
    await expect(mainWindow).toHaveTitle(/Node-RED-Desktop/);
  });
